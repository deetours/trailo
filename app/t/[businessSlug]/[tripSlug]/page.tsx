...
import TripEnquiryAction from '@/components/trip-page/TripEnquiryAction';
      ...
      <section className={`pt-24 pb-12 px-6 md:px-10 max-w-5xl mx-auto ${isEditorial ? 'text-center' : ''}`}>
        {content.heroImage ? (
          <div className={`w-full ${isEditorial ? 'h-[60vh] md:h-[80vh] rounded-sm' : isBold ? 'h-[50vh] rounded-none' : 'h-[40vh] md:h-[60vh] rounded-2xl'} overflow-hidden mb-8 relative`}>
            <Image src={content.heroImage} alt={trip.basicInfo.name} fill sizes="(min-width: 768px) 900px, 100vw" className="object-cover" />
        ...
        <Reveal className={isEditorial ? 'max-w-4xl mx-auto' : 'max-w-3xl'}>
          <h1 className={`text-display-lg tracking-tight mb-4 ${headerFont}`}>
            {trip.basicInfo.name}
          </h1>
      ...
      {trip.pricing && trip.pricing.tiers && trip.pricing.tiers.length > 0 && (
        <section id="pricing" className={`py-12 px-6 md:px-10 max-w-5xl mx-auto border-t border-border mt-12 ${isEditorial ? 'text-center' : ''}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {trip.pricing.tiers.map((tier: any, i: number) => (
              <Reveal key={tier.id} delay={0.1 * i} variant="fade">
                <div className={`border border-border bg-card p-6 flex flex-col h-full ${isBold ? 'rounded-none border-2' : isEditorial ? 'rounded-sm' : 'rounded-2xl'}`}>
